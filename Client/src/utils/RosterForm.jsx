...
//   const { roster, loading, error, allRosters, rosterDetailLoading } = useSelector((state) => state.roster || {});
...
//     } catch (err) {
//       console.error(err);
//       alert("Failed to load saved roster data");
//     }
//   };
...
//     if (!employeeInput.name) return alert("Enter employee name");

//     if (!employeeInput.isCoreTeam) {
//       if (!employeeInput.shift) return alert("Select shift for non-core team member");
//       if (!employeeInput.shiftStartHour) return alert("Enter shift start hour");
//       if (!employeeInput.shiftEndHour) return alert("Enter shift end hour");

//       const startHour = parseInt(employeeInput.shiftStartHour);
//       const endHour = parseInt(employeeInput.shiftEndHour);

//       if (isNaN(startHour) || isNaN(endHour)) {
//         return alert("Shift hours must be numbers");
//       }
//     }
...
//     if (!employees.length) return alert("Add at least one employee");
...
//     } catch (err) {
//       console.error(err);
//       alert(err.message || "Failed to generate roster");
//     }
//   };
...
//       if (!rosterObj) return alert("Roster context missing");
...
//     alert("Remove from saved roster is not implemented here");
//   };
...
//                   <th className="border p-3 text-left font-semibold text-gray-800">Core Team</th>
//                   <th className="border p-3 text-left font-semibold text-gray-800">Shift Type</th>
...
//                         <span className={`px-2 py-1 rounded text-xs ${emp.isCoreTeam ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>
//                           {emp.isCoreTeam ? "Yes" : "No"}
//                         </span>
//                       </td>
//                       <td className="border p-3 text-gray-800">{emp.shift || (emp.isCoreTeam ? "N/A" : "-")}</td>
//                       <td className="border p-3 text-gray-800">
...
//                                 <div
//                                   key={dayIndex}
//                                   className={`w-8 h-8 flex items-center justify-center rounded border cursor-pointer ${
//                                     status === "P" ? "bg-green-100 border-green-300" :
...
//                           {/* <button onClick={() => handleRemoveSaved(emp._id, rosterWeek.weekNumber)} className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-sm">Remove</button> */}
//                         </div>
//                       </td>
//                     </tr>
//                   );
...
//                       <input type="checkbox" name="isCoreTeam" id="isCoreTeam" checked={employeeInput.isCoreTeam} onChange={handleInputChange} className="mr-2 h-5 w-5" />
//                       <label htmlFor="isCoreTeam" className="text-gray-800">Core Team Member</label>
//                     </div>

//                     {!employeeInput.isCoreTeam && (
//                       <>
//                         <select name="shift" value={employeeInput.shift} onChange={handleInputChange} className="border p-3 rounded text-gray-800" required>
//                           <option value="" className="text-gray-500">Select Shift Type *</option>
//                           <option value="Start" className="text-gray-800">Start Shift</option>
//                           <option value="Mid" className="text-gray-800">Mid Shift</option>
//                           <option value="End" className="text-gray-800">End Shift</option>
//                         </select>
...
//                                 <td className="border p-3 text-gray-800">{emp.transport || "-"}</td>
//                                 <td className="border p-3 text-gray-800">{emp.cabRoute || "-"}</td>
//                                 <td className="border p-3 text-gray-800"><span className={`px-2 py-1 rounded text-xs ${emp.isCoreTeam ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>{emp.isCoreTeam ? "Yes" : "No"}</span></td>
//                                 <td className="border p-3 text-gray-800">{emp.shift || (emp.isCoreTeam ? "N/A" : "-")}</td>
...
//                       <button onClick={handleGenerateRoster} disabled={loading || employees.length === 0} className={`px-6 py-3 rounded font-medium ${loading || employees.length === 0 ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "bg-green-500 hover:bg-green-600 text-white cursor-pointer"}`}>
//                         {loading ? <span className="flex items-center"><svg className="animate-spin h-5 w-5 mr-2" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>Generating...</span> : "Generate Roster"}
//                       </button>
...
const RosterForm = () => {
  ...
  const [showSaveModal, setShowSaveModal] = useState(false); // ADD THIS
  const [saveAction, setSaveAction] = useState("create"); // ADD THIS: "create" or "add"
  ...
  const executeExportSavedRoster = async () => {
    try {
      ...
      alert("Saved roster exported successfully!");
      setShowExportModal(false);
    } catch (err) {
      console.error(err);
      alert(err.message || "Failed to export saved roster");
    }
  };
  ...
  // MODIFIED: Open modal instead of directly saving
  const handleSaveRoster = () => {
    if (!employees.length) return alert("Add at least one employee");
    setShowSaveModal(true);
  };

  // NEW: Function to actually save roster with action type
  const executeSaveRoster = async () => {
    try {
      ...
      await dispatch(
        addRosterWeek({
          data: {
            ...
            action: saveAction, // ADD THIS - send action to backend
          },
        })
      ).unwrap();
      ...
      // Reset form if it's a create action
      if (saveAction === "create") {
        setEmployees([]);
      }
      
      // Refresh saved rosters list
      await dispatch(fetchAllRosters({})).unwrap();
  ...
  return (
    <>
      ...
      {/* ADD THIS NEW MODAL FOR SAVE OPTIONS */}
      {showSaveModal && (
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-black bg-opacity-40">
