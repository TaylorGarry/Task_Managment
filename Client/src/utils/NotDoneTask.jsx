...
const NotDoneTask = () => {
  ...
  const doneTasksCount = taskList.filter((task) =>
    isPrivilegedUser
  ...
  const notDoneTasksCount = Math.max(taskList.length - doneTasksCount, 0);
...
<<<<<<< HEAD
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex justify-between items-start">
        <div>
          <p className="text-2xl font-stretch-50% font-semibold text-blue-500 mb-2">Not Done Task</p>
...
// const NotDoneTask = ({ notDoneTasks, loading, error }) => {
//   const [displayCount, setDisplayCount] = useState(0);
//   const [isAnimating, setIsAnimating] = useState(false);

//   useEffect(() => {
//     // Reset animation when component receives new data
//     if (!loading && !error) {
//       setIsAnimating(true);
//       setDisplayCount(0);
...
//       const duration = 1500;
//       const steps = 50;
//       const increment = notDoneTasks / steps;
//       let current = 0;
      
//       const timer = setInterval(() => {
//         current += increment;
//         if (current >= notDoneTasks) {
//           setDisplayCount(notDoneTasks);
//           setIsAnimating(false);
//           clearInterval(timer);
//         } else {
//           setDisplayCount(Math.floor(current));
