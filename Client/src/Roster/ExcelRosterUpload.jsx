...
//   const getMinDate = () => {
//     if (isSuperAdmin) return '2020-01-01';
//     if (isAllowedDepartmentEmployee) return tomorrowFormatted;
...
//     if (!canUpload) {
//       toast.error('You do not have permission to upload roster');
//       return;
//     }
...
//     if (!canUpload) {
//       toast.error('You do not have permission to export or upload roster templates');
//       return;
//     }

//     if (!startDate || !endDate) {
//       toast.error('Please select both start and end dates first');
//       return;
//     }
...
//     await dispatch(exportRosterTemplate({
//       startDate,
//       endDate
//     }));
//   };
...
const ExcelRosterUpload = () => {
  ...
  const isSuperAdmin = userData?.accountType === 'superAdmin';
  const isAdmin = ['admin', 'HR'].includes(userData?.accountType);
  const allowedRosterDepartments = ['Ops - Meta', 'Marketing', 'CS'];
  const isAllowedDepartmentEmployee =
    userData?.accountType === 'employee' &&
    allowedRosterDepartments.includes(userData?.department);
  const canUpload = isSuperAdmin || isAdmin || isAllowedDepartmentEmployee;
  ...
  const validateForm = () => {
    ...
    if (!endDate) {
      ...
      // Check if it's exactly 7 days for non-superAdmin users
      if (!isSuperAdmin) {
        const diffTime = Math.abs(end - start);
        const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)) + 1;
 ...
 const validateExportTemplate = () => {
  ...
  const diffTime = Math.abs(end - start);
  const diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24)) + 1; // Use Math.floor instead of Math.ceil
  
  console.log('Selected Start:', startDate);
  console.log('Selected End:', endDate);
  console.log('Total Days:', diffDays);
  
  // For non-superAdmin users, check if it's exactly 7 days
  if (!isSuperAdmin) {
    if (diffDays !== 7) {
      toast.error(`Date range must be exactly 7 days. You selected ${diffDays} days.`);
      return false;
    }
  }
  ...
  const handleExportTemplate = async () => {
    ...
    if (!validateExportTemplate()) {
      return;
    }

    const result = await dispatch(exportRosterTemplate({
      startDate,
      endDate
    }));
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-6">
      ...
      <div className="mb-8">  
        ...
        {/* <p className="text-gray-600">
          {isSuperAdmin 
            ? 'SuperAdmin can upload roster for any date range' 
                ...
                <div>
                  ...
                  {validationErrors.endDate ? (
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      {isSuperAdmin 
                        ? 'SuperAdmin can select any date range ' 
      ...
      <div className="mt-8 p-4 bg-gray-50 rounded-xl">
        <div className="text-sm text-gray-600">
          ...
          Role: <strong>{userData?.accountType}</strong> • 
          ...
          {isSuperAdmin && (
            <span className="ml-2 px-2 py-1 text-xs font-medium bg-purple-100 text-purple-800 rounded-full">
              SuperAdmin (Unlimited Date Range)
            </span>
          )}
        </div>
      </div>
