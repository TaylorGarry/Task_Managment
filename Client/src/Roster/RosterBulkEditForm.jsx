...
const RosterBulkEditForm = ({ rosterId, onClose }) => {
  ...
  // Initialize edited weeks when data loads
  useEffect(() => {
  ...
  // Add new employee to current and future weeks
  const handleAddNewEmployee = () => {
    ...
    // Add employee to current and future weeks only
    updatedWeeks.forEach((week, weekIndex) => {
      const weekStartDate = new Date(week.startDate);
      
      // Check if user is superAdmin (can add to past weeks) or week is current/future
      ...
      if (isSuperAdmin || isCurrentOrFuture) {
        ...
        if (!employeeExists) {
          // Create daily status array for the week
          const dailyStatus = [];
          ...
          while (currentDate <= endDate) {
            // Use corresponding status from new employee, or default to "P"
  ...
  // Remove employee from specific week
            ...
            {editedWeeks.map((week, index) => {
              ...
              return (
                <button
                  ...
                  className={`px-4 py-3 border-b-2 font-medium whitespace-nowrap flex items-center ${
                    activeTab === index
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gray-50">
                      <th className="p-3 text-left font-semibold text-gray-800 border-b">Name</th>
                      <th className="p-3 text-left font-semibold text-gray-800 border-b">Transport</th>
                      <th className="p-3 text-left font-semibold text-gray-800 border-b">CAB Route</th>
                      <th className="p-3 text-left font-semibold text-gray-800 border-b">Shift Hours</th>
                      <th className="p-3 text-left font-semibold text-gray-800 border-b">Daily Status</th>
                      <th className="p-3 text-left font-semibold text-gray-800 border-b">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                                ...
                                return (
                                  <div key={dayIndex} className="flex flex-col items-center">
                                    ...
                                    <select
                                      ...
                                      className={`w-10 h-10 border rounded text-center text-sm ${
                                    ...
                                    <span className="text-xs mt-1">{getStatusIcon(status.status)}</span>
                                  </div>
                                );
                              })}
                            </div>
