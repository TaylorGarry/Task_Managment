...
//   { value: "OT", label: "Over Time (OT)", color: "bg-indigo-100 text-indigo-800" },
//   { value: "FWO", label: "Force WeekOFF (FWO)", color: "bg-slate-200 text-slate-800" },
//   { value: "EXIT", label: "Exit (EXIT)", color: "bg-orange-100 text-orange-800" }
// ];
...
// const mergeWeeksStable = (existingWeeks = [], incomingWeeks = []) => {
//   const byKey = new Map();
//   [...existingWeeks, ...incomingWeeks].forEach((week) => {
//     const startKey = toIstDateKey(week?.startDate) || "";
//     const endKey = toIstDateKey(week?.endDate) || "";
//     const requestWeekNumber = String(week?.actualWeekNumber ?? week?.weekNumber ?? "");
//     const key = `${requestWeekNumber}|${startKey}|${endKey}`;
//     if (!key) return;
//     byKey.set(key, week);
//   });
...
//   const { updateEmployeesData, allRosters, loading, error } = useSelector((state) => state.roster);
  
//   const initialFetchDone = useRef(false);
//   const lastEmployeesFetchKeyRef = useRef("");
//   const autoSelectWeekDone = useRef(false); // ✅ NEW
//   const weekDateMemoryRef = useRef({});
...
//   const fullRosterWeeks = useMemo(() => {
//     const rosterList = Array.isArray(allRosters?.data) ? allRosters.data : [];
//     const activeId = String(activeRosterId || rosterId || "");
//     if (!activeId) return [];

//     const matchedRoster = rosterList.find((r) => String(r?._id || r?.rosterId || "") === activeId);
//     const weeks = Array.isArray(matchedRoster?.weeks) ? matchedRoster.weeks : [];
//     const normalized = monthFilterContext
//       ? weeks
//           .filter((week) =>
...
//       return aStart - bStart;
//     });
//   }, [allRosters, activeRosterId, rosterId, monthFilterContext]);
...
//     const nextKey = JSON.stringify(requestParams);
//     if (lastEmployeesFetchKeyRef.current === nextKey) return;
//     lastEmployeesFetchKeyRef.current = nextKey;

//     dispatch(getEmployeesForUpdates(requestParams));
...
//       }
//       // Keep roster selection stable; updates endpoint can return adjacent rosterId and collapse week list.
//       if (!rosterId && responseData.rosterId && String(responseData.rosterId) !== String(activeRosterId)) {
//         setActiveRosterId(responseData.rosterId);
//       }
      
//       const responseWeeks = responseData.weeks || [];
//       const incomingWeeks = monthFilterContext
//         ? responseWeeks
//             .filter((week) =>
//               isWeekOverlappingMonth(week, monthFilterContext.month, monthFilterContext.year)
//             )
...
//       // updates API can return only current week; fallback to full roster weeks from rosterdetail cache.
//       const baseWeeks = fullRosterWeeks.length > 1 ? fullRosterWeeks : availableWeeks;
//       const sortedWeeks = mergeWeeksStable(baseWeeks, incomingWeeks);
...
//       }
//     }
//   }, [updateEmployeesData, availableWeeks, fullRosterWeeks, selectedDate, selectedWeek, monthFilterContext, activeRosterId]);
...
//     weekDateMemoryRef.current[String(selectedWeek)] = selectedDate;
...
//                   if (rememberedDate && isDateWithinWeek(rememberedDate, nextWeek)) {
//                     setSelectedDate(rememberedDate);
//                   } else {
...
//   ...STATUS_OPTIONS,
//   { value: "Self", label: "Self", color: "bg-cyan-100 text-cyan-800" },
//   { value: "Did Not Board", label: "Did Not Board", color: "bg-red-100 text-red-800" },
//   { value: "Cab Moved", label: "Cab Moved", color: "bg-blue-100 text-blue-800" },
// ];
...
// // ✅ NEW: Normalize shift string for searching
// const normalizeShiftForSearch = (shiftString) => {
//   if (!shiftString) return '';
  
//   // Remove "00" from hours in format "6:00 - 3:00" or "18:00 - 3:00"
//   let normalized = shiftString.replace(/:00/g, '');
...
// const getEquivalentSearchValues = (value) => {
//   const raw = String(value || "").trim().toLowerCase();
//   const normalized = normalizeShiftForSearch(raw);
//   if (!raw && !normalized) return [];

//   const values = new Set([raw, normalized].filter(Boolean));

//   // Search-only 12h/24h shift equivalence. Example: "11-8" also searches
//   // "23-8", and "18-3" also searches "6-3"; saved roster data is untouched.
//   const shiftPairMatch = normalized.match(/^(\d{1,2})-(\d{1,2})$/);
//   if (shiftPairMatch) {
//     const startHour = Number.parseInt(shiftPairMatch[1], 10);
//     const endHour = Number.parseInt(shiftPairMatch[2], 10);
//     if (
...
//       endHour <= 23
//     ) {
//       const equivalentStartHour = startHour >= 12 ? startHour - 12 : startHour + 12;
//       values.add(`${equivalentStartHour}-${endHour}`);
//     }
//   }
...
// const matchesShiftSearch = (shiftValue, searchValue) => {
//   const shiftLower = String(shiftValue || "").toLowerCase();
//   const normalizedShift = normalizeShiftForSearch(shiftLower);
//   const searchValues = getEquivalentSearchValues(searchValue);

//   return searchValues.some((value) => {
//     const normalizedValue = normalizeShiftForSearch(value);
//     return (
//       shiftLower.includes(value) ||
//       normalizedShift.includes(value) ||
//       (normalizedValue && normalizedShift.includes(normalizedValue))
//     );
//   });
// };

// const getStatusColor = (status) => {
//   const option = BULK_STATUS_OPTIONS.find(opt => opt.value === status);
//   return option?.color || "bg-gray-100 text-gray-800";
// };
...
//   const initialFetchDone = useRef(false);
//   const lastEmployeesFetchKeyRef = useRef("");
//   const autoSelectWeekDone = useRef(false);
//   const weekDateMemoryRef = useRef({});
...
//   // ✅ MODIFIED: Filter roster entries based on search
//   const filteredRosterEntries = useMemo(() => {
//     if (!appliedSearch) return rosterEntries;
    
//     const searchLower = appliedSearch.toLowerCase().trim();
//     if (!searchLower) return rosterEntries;
    
//     return rosterEntries.filter((employee) => {
//       if (!employee) return false;
      
//       const name = String(employee.name || "").toLowerCase();
//       const department = String(employee.department || "").toLowerCase();
//       const teamLeader = String(employee.teamLeader || "").toLowerCase();
//       const shift = formatShift(employee.shiftStartHour, employee.shiftEndHour);
//       const shiftLower = shift.toLowerCase();
      
//       // For shift search, normalize both sides for comparison
//       if (searchBy === "shift") {
//         return matchesShiftSearch(shiftLower, searchLower);
//       }
      
//       // For other search fields
//       if (searchBy === "name") {
...
//         return teamLeader.includes(searchLower);
//       }
      
//       // "all" search - check all fields
//       return name.includes(searchLower) ||
//              department.includes(searchLower) ||
//              teamLeader.includes(searchLower) ||
//              shiftLower.includes(searchLower) ||
//              matchesShiftSearch(shiftLower, searchLower);
//     });
//   }, [rosterEntries, appliedSearch, searchBy]);

//   const pagination = updateEmployeesData?.data?.pagination;
//   const totalEmployees = Number(pagination?.totalEmployees ?? filteredRosterEntries.length);
//   const totalPages = Number(
//     pagination?.totalPages ?? Math.max(1, Math.ceil(totalEmployees / Math.max(pageSize, 1)))
//   );
//   const rangeStart = totalEmployees === 0 ? 0 : (currentPage - 1) * pageSize + 1;
...
//   useEffect(() => {
//     const currentIds = new Set(filteredRosterEntries.map((e) => String(e._id)));
//     setSelectedEmployeeIds((prev) => {
//       const newSelected = prev.filter((id) => {
//         if (!currentIds.has(String(id))) return false;
//         const row = filteredRosterEntries.find((emp) => String(emp._id) === String(id));
//         return row ? !isOwnRosterRow(row) : false;
//       });
//       if (newSelected.length === prev.length && newSelected.every((id, i) => id === prev[i])) {
//         return prev;
...
//   const currentUserId = String(currentUser?._id || currentUser?.id || "").trim();
//   const currentUsername = String(currentUser?.username || "").trim().toLowerCase();
//   const inferredManagedTeamCount = filteredRosterEntries.filter((employee) => {
//     if (!employee) return false;
//     const sameUserId =
//       currentUserId &&
//       employee.userId &&
...
//     return Boolean(sameUserId || sameName || sameUsername);
//   };
//   const selectableEntries = filteredRosterEntries.filter((employee) => !isOwnRosterRow(employee));
...
//             <div className="bg-gray-50 rounded-lg p-3">
//               <p className="text-sm text-gray-600">
//                 <span className="font-semibold">Employees:</span> {filteredRosterEntries.length}
//               </p>
//               {appliedSearch ? (
//                 <p className="text-xs text-gray-500 mt-1">
//                   Showing results for: <span className="font-medium">{appliedSearch}</span>
...
//         {/* Employees Table */}
//         {loading && filteredRosterEntries.length === 0 ? (
//           <div className="bg-white rounded-lg shadow p-8 text-center">
//             <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-emerald-600 mx-auto"></div>
//             <p className="mt-4 text-gray-600">Loading employees...</p>
//           </div>
...
//                   </thead>
//                   <tbody className={isDarkTable ? "bg-[#13214f]" : "bg-white"}>
//                   {filteredRosterEntries.map((employee) => {
//                     const todayStatus = getTodayStatus(employee);
//                     const isSelected = selectedEmployeeIds.some((id) => String(id) === String(employee._id));
//                     const isOwnRow = isOwnRosterRow(employee);
...
//                 <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
//                   <span style={{ padding: "6px 12px", borderRadius: "999px", background: "#ecfeff", border: "1px solid #a5f3fc", color: "#0e7490", fontWeight: 600 }}>
//                     Employees: {filteredRosterEntries.length}
//                   </span>
//                 </div>
//               </div>
...
//                   </thead>
//                   <tbody>
//                     {filteredRosterEntries.map((emp, idx) => {
//                       const attendance = getTodayStatus(emp);
//                       const values = {
//                         employee: emp?.name || "-",
//                         teamLeader: emp?.teamLeader || "-",
...
const ArrivalAttendanceUpdate = ({ rosterId, delegatedFromUserId = "" }) => {
  ...
  const handleExcelUpload = async (event) => {
    ...
    try {
      ...
      // Use the bulkUpdatePunchTimes endpoint with file upload
      await dispatch(bulkUpdatePunchTimes(formData)).unwrap();

      toast.success('Excel punch attendance uploaded successfully!');
      setExcelFile(null);
      fileInputRef.current.value = '';
  ...
  // ✅ Check if user can upload Excel (HR, Super Admin, IT)
  const canUploadExcel = viewType.isHR || viewType.isSuperAdmin || viewType.department === "IT";
    ...
    <div className="bg-gray-100 overflow-x-hidden">
      ...
      <div className="container mx-auto w-full max-w-full px-4 py-8">
            ...
            <div className="bg-white rounded-lg shadow p-4">
              <div className="flex flex-wrap items-end gap-4">
                ...
                <div className="flex items-center gap-2 ml-auto">
                  ...
                  {/* ✅ NEW: Excel Upload Button - Only for HR, Super Admin, IT */}
                  {canUploadExcel && (
                    <>
                      <input
                        type="file"
        ...
        <div className="mt-6 bg-white rounded-lg shadow p-4">
          ...
          {/* ✅ NEW: Excel Upload Info */}
          {canUploadExcel && (
            <div className="mt-3 pt-3 border-t border-gray-200">
              <p className="text-xs text-blue-600">
                <span className="inline-block w-2 h-2 rounded-full bg-blue-500 mr-1"></span>
                Excel Upload: Upload Monthly Status Report (.xlsx/.xls) to bulk update punch times
              </p>
            </div>
          )}
        </div>
