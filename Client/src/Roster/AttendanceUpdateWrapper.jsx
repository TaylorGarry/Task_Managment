...
const AttendanceUpdateWrapper = ({ delegatedMode = false }) => {
  ...
  const visibleRosters = useMemo(() => {
    ...
    if (normalizedRosters.length <= 1) {
      return normalizedRosters;
    }

    const sortByWeekStart = (a, b) => {
      const aStart = new Date(a?.displayStartDate || a?.startDate || 0).getTime();
      const bStart = new Date(b?.displayStartDate || b?.startDate || 0).getTime();
      return aStart - bStart;
    };

    const sortedRosters = [...normalizedRosters].sort((a, b) => {
      const aStart = new Date(a?.displayStartDate || a?.rosterStartDate || 0).getTime();
      const bStart = new Date(b?.displayStartDate || b?.rosterStartDate || 0).getTime();
      return aStart - bStart;
    });

    const primaryRoster =
      sortedRosters.find((roster) =>
        (roster.weeks || []).some((week) => {
          const startKey = toIstKey(week?.displayStartDate || week?.startDate);
          const endKey = toIstKey(week?.displayEndDate || week?.endDate);
          if (!startKey || !endKey) return false;
    ...
    sortedRosters.forEach((roster) => {
      (roster.weeks || []).forEach((week) => {
        const weekStartKey = toIstKey(week?.displayStartDate || week?.startDate);
        const weekEndKey = toIstKey(week?.displayEndDate || week?.endDate);
        if (!weekStartKey || !weekEndKey) return;

        const weekKey = `${weekStartKey}|${weekEndKey}`;
        ...
        const existingWeekEmployeeKeys = new Set(
          (mergedWeek.employees || []).map((emp) =>
            String(emp?.userId?._id || emp?.userId || emp?.empId || `${emp?.name || ""}|${emp?.department || ""}`).trim()
          )
        );

        (Array.isArray(week?.employees) ? week.employees : []).forEach((emp) => {
          if (!emp) return;
          const employeeKey = String(
            emp?.userId?._id || emp?.userId || emp?.empId || `${emp?.name || ""}|${emp?.department || ""}`
          ).trim();
          if (!employeeKey || existingWeekEmployeeKeys.has(employeeKey)) return;

          existingWeekEmployeeKeys.add(employeeKey);
          mergedWeek.employees.push(emp);
          uniqueEmployeeKeys.add(employeeKey);

          if (emp?.teamLeader) {
            teamLeaders.add(emp.teamLeader);
          }
        });
      });
    });

    const mergedWeeks = Array.from(mergedWeeksMap.values()).sort(sortByWeekStart);
    if (!mergedWeeks.length) {
      return normalizedRosters;
    }

    const displayStartDate = mergedWeeks
      .map((week) => new Date(week.displayStartDate || week.startDate))
      .sort((a, b) => a - b)[0]
      ?.toISOString();
    const displayEndDate = mergedWeeks
      .map((week) => new Date(week.displayEndDate || week.endDate))
      .sort((a, b) => b - a)[0]
      ?.toISOString();
    ...
    return [
      {
        ...
        teamLeaders: Array.from(teamLeaders),
        displayStartDate,
        displayEndDate,
      },
    ];
          ...
          {(loading || rosterDetailLoading) ? (
            <div className="text-center py-12">
              ...
              {visibleRosters.map((roster) => {
                const totalEmployees = Number.isFinite(roster?.totalEmployees)
                  ? roster.totalEmployees
                  : (roster.weeks?.reduce((sum, week) =>
                ...
                const rosterTeamLeaders = Array.isArray(roster?.teamLeaders) && roster.teamLeaders.length > 0
                  ? roster.teamLeaders
                  : [...new Set(
                      (roster.weeks || [])
                        .flatMap((week) => Array.isArray(week?.employees) ? week.employees : [])
                        .map((employee) => employee?.teamLeader)
                        .filter(Boolean)
                    )];
                ...
                return (
                    ...
                    onClick={() => {
	                  ...
	                  >
                        ...
                        {rosterTeamLeaders.length > 0 && (
	                          <div className="mt-3 text-xs text-slate-500 leading-relaxed">
	                            Team: {rosterTeamLeaders.join(', ')}
	                          </div>
	                        )}
	                      </div>
	                    </div>
