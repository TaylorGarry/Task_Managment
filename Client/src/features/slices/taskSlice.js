...
export const createCoreTask = createAsyncThunk(
  ...
  async ({ data }, thunkAPI) => {
    try {
      ...
      // Invalidate cache when new core task is created
...
export const updateTaskStatus = createAsyncThunk(
  ...
  async ({ id, status }, thunkAPI) => {
    try {
      ...
      // Invalidate cache when task status is updated
      cache.tasks.data = null;
      cache.tasks.timestamp = null;
...
export const updateTaskStatusCoreTeam = createAsyncThunk(
  ...
  async ({ id, status }, thunkAPI) => {
    try {
      ...
      // Invalidate cache when core task status is updated
      cache.coreTasks.data = null;
      cache.coreTasks.timestamp = null;
...
const taskSlice = createSlice({
  ...
  reducers: {
    // Add a reducer to manually invalidate cache if needed
