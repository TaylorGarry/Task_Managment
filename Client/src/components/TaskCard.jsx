...
const TaskCard = ({ task }) => {
  ...
  const renderEmployeeBadge = (emp, color) => {
    if (!emp || !emp.username) return null;
    return (
      <div key={emp._id || Math.random()} className="flex items-center gap-2">
        <div
        ...
        <div className="mb-3">
          ...
          <div className="flex flex-wrap gap-2">
            {doneEmployees.length > 0
              ? doneEmployees.map((emp) => renderEmployeeBadge(emp, "bg-green-500"))
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {notDoneEmployees.length > 0
              ? notDoneEmployees.map((emp) => renderEmployeeBadge(emp, "bg-red-500"))
              : <span className="text-sm text-gray-500">None</span>}
          </div>
        </div>
      </div>
