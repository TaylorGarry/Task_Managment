...
import { fetchEmployeeDashboardSummary, updateProfile } from "../features/slices/authSlice";
import Navbar from "./Navbar";
...
const MyProfile = () => {
  ...
  const [policyLoading, setPolicyLoading] = useState(false);
  const [policyLoadError, setPolicyLoadError] = useState("");
  const [accountDetails, setAccountDetails] = useState(ACCOUNT_DETAILS_INITIAL);
  const [savingAccountDetails, setSavingAccountDetails] = useState(false);

  useEffect(() => {
    if (user?.accountType === "employee" || user?.roleType === "agent" || user?.roleType === "supervisor") {
      dispatch(fetchEmployeeDashboardSummary());
    }
  }, [dispatch, user?.accountType, user?.roleType]);
  ...
  const handleSaveAccountDetails = async (e) => {
    ...
    try {
      await dispatch(updateProfile({ accountDetails })).unwrap();
      await dispatch(fetchEmployeeDashboardSummary()).unwrap();
      toast.success("Account details saved successfully");
    } catch (err) {
      toast.error(err || "Failed to save account details");
    } finally {
        ...
        {profile && (
          ...
          >
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-cyan-700">Account Details</p>
                <p className="mt-1 text-sm text-slate-500">Your PAN, Aadhaar, UAN and bank details.</p>
              </div>
              <button
                type="submit"
                disabled={savingAccountDetails}
                className="rounded-lg bg-cyan-600 px-4 py-2 text-sm font-semibold text-white hover:bg-cyan-700 disabled:cursor-not-allowed disabled:bg-slate-300"
              >
              ...
              {ACCOUNT_DETAIL_FIELDS.map((field) => (
                <div key={field.name}>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 outline-none transition focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100"
                  />
                </div>
              ))}
            </div>
