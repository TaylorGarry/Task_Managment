...
//   TOTAL: 1.5,
// });
// const MONTHLY_CREDIT_BEFORE_JULY_2026 = 2;
// const MONTHLY_CREDIT_FROM_JULY_2026 = 1.5;
...
//   let totalCredit = 0;
//   let monthsToCredit = 0;
//   for (let monthCursor = new Date(effectiveStart); monthCursor <= targetMonth; monthCursor = addMonths(monthCursor, 1)) {
//     const currentMonthStart = startOfMonth(monthCursor);
//     totalCredit += currentMonthStart >= JULY_2026_MONTH_START
//       ? MONTHLY_CREDIT_FROM_JULY_2026
//       : MONTHLY_CREDIT_BEFORE_JULY_2026;
//     monthsToCredit += 1;
//   }

//   return {
//     monthsToCredit,
//     totalCredit: roundToHalf(totalCredit),
...
//   const startDay = startOfDay(startDate).getDay();
//   const endDay = startOfDay(endDate).getDay();
//   const crossesWeekendBetweenLeaveDays = startDay <= 5 && endDay >= 1 && weekendCount > 0;
...
//   return {
//     totalDays: days.length,
//     sandwichDays: crossesWeekendBetweenLeaveDays ? weekendCount : 0,
...
export const getFinancialYearBounds = (referenceDate = new Date()) => {
  const ref = startOfDay(referenceDate);
  const refParts = getIstParts(ref);
  const year = refParts.month < LEAVE_CYCLE_START_MONTH_INDEX + 1 ? refParts.year - 1 : refParts.year;
  const start = toIstDate(year, LEAVE_CYCLE_START_MONTH_INDEX + 1, 1, 0, 0, 0, 0);
  const nextStart = toIstDate(year + 1, LEAVE_CYCLE_START_MONTH_INDEX + 1, 1, 0, 0, 0, 0);
  const end = new Date(nextStart.getTime() - 1);
  return { start, end, startYear: year };
...
export const computeSandwichAndChargeDays = ({
  ...
  const days = getDatesInRange(startDate, endDate);
  const weekendCount = days.filter((d) => d.getDay() === 0 || d.getDay() === 6).length;
  const providedWeekoffCount = Number(weekoffCount);
  const hasWeekoffCount = Number.isFinite(providedWeekoffCount);
  const startBoundary = normalizeStartSession(startSession);
  const endBoundary = normalizeEndSession(endSession);
  ...
  return {
    totalDays: days.length,
    sandwichDays: hasWeekoffCount ? roundToHalf(Math.max(0, providedWeekoffCount)) : weekendCount,
    chargedDays: roundToHalf(chargedDays),
    invalid: false,
    normalizedStartSession: startBoundary,
    normalizedEndSession: endBoundary,
